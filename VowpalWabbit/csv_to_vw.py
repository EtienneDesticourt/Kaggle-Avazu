  ...
  with open(loc_output,"wb") as outfile:
    for e, row in enumerate( DictReader(open(loc_csv)) ):
      ...
      if train: #we care about labels
        ...
        outfile.write( bytes("%s '%s |i%s |c%s\n" % (label,row['id'],numerical_features,categorical_features), "UTF-8") )
		
      else: #we dont care about labels
        outfile.write( bytes("1 '%s |i%s |c%s\n" % (row['id'],numerical_features,categorical_features), "UTF-8") )
...
csv_to_vw("../data/train.csv", "train.vw",train=True)
csv_to_vw("../data/test.csv", "test.vw",train=False)
